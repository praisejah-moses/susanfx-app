...
export default function SettingsPage() {
  ...
  const { prefs, loading: prefsLoading, save: savePrefs } = useNotificationPrefs(user?.id);
  ...
  const [payoutAlerts, setPayoutAlerts] = useState(prefs?.payout_alerts ?? false);
  ...
  async function handleProfileSave(e: React.FormEvent) {
    ...
    const { error } = await supabase.auth.updateUser({
      data: { first_name: firstName, last_name: lastName, phone_number: phone, country },
    });
  ...
  async function handleNotifSave(key: "emailNotifs" | "tradeAlerts" | "payoutAlerts", val: boolean) {
    const next = {
      ...
      <div className="md:ml-60 flex flex-col min-h-screen">
        ...
        <main className="flex-1 px-4 md:px-8 py-5 md:py-8 space-y-6 animate-fadeInUp max-w-2xl">
          ...
          <section className="bg-[#0f0f0f] border border-(--border-normal) rounded-xl p-5 md:p-6 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="flex flex-col gap-1.5">
                <label className="text-xs text-(--text-white-50)">First Name</label>
                <input
                  type="text"
                  value={firstName}
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-xs text-(--text-white-50)">Last Name</label>
                <input
                  type="text"
                  value={lastName}
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-xs text-(--text-white-50)">Email</label>
                <input
                  type="email"
                  value={user?.email ?? ""}
                  disabled
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-xs text-(--text-white-50)">Phone</label>
                <input
                  type="tel"
                  value={phone}
              ...
              <div className="flex flex-col gap-1.5 sm:col-span-2">
                <label className="text-xs text-(--text-white-50)">Country</label>
                <input
                  type="text"
                  value={country}
          ...
          <section className="bg-[#0f0f0f] border border-(--border-normal) rounded-xl p-5 md:p-6 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="flex flex-col gap-1.5">
                <label className="text-xs text-(--text-white-50)">New Password</label>
                <input
                  type="password"
                  value={newPassword}
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-xs text-(--text-white-50)">Confirm Password</label>
                <input
                  type="password"
          ...
          <section className="bg-[#0f0f0f] border border-(--border-normal) rounded-xl p-5 md:p-6 space-y-4">
            ...
            <div className="space-y-3">
              {!prefsLoading && [
                { label: "Email Notifications",  sub: "Receive account updates via email",    value: emailNotifs,  key: "emailNotifs"  as const },
                { label: "Trade Alerts",          sub: "Get alerts on trade executions",        value: tradeAlerts,  key: "tradeAlerts"  as const },
                { label: "Payout Notifications",  sub: "Notify when payout is processed",      value: payoutAlerts, key: "payoutAlerts" as const },
              ].map((n) => (
                <div key={n.label} className="flex items-center justify-between gap-4">
                  <div>
                    <p className="text-sm font-medium text-(--global-text)">{n.label}</p>
                  ...
                  >
                    <span className={`absolute top-0.5 left-0.5 w-5 h-5 rounded-full bg-white transition-transform ${n.value ? "translate-x-5" : "translate-x-0"}`} />
                  </button>
                </div>
              ))}            </div>
          </section>
          ...
          <section className="bg-[#0f0f0f] border border-red-500/20 rounded-xl p-5 md:p-6 space-y-3">
            ...
            <p className="text-xs text-(--text-white-50)">Permanently close your account. This action cannot be undone.</p>

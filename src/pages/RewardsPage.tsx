              ...
              {rewards.map((reward) => (
                ...
                >
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 text-lg ${
                    reward.earned ? "bg-(--primary-default)/10" : "bg-white/5"
                  }`}>
                    {reward.earned ? "🏆" : "🔒"}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-(--global-text)">{reward.title}</p>
                    <p className="text-xs text-(--text-white-50) mt-0.5">{reward.description}</p>
                    <p className={`text-xs font-medium mt-2 ${reward.earned ? "text-(--primary-default)" : "text-(--text-white-50)"}`}>
                      {reward.points.toLocaleString()} pts {reward.earned ? "· Earned" : ""}
                    </p>
                  </div>
                </div>
              ))}
              ...
              <table className="w-full text-xs">
                ...
                <tbody className="divide-y divide-(--border-normal)">
                  {loading ? (
                    ...
                    <tr><td colSpan={4} className="px-5 py-8 text-center text-(--text-white-50)">No data yet</td></tr>
                    ...
                    const displayName = isMe
                      ...
                      : entry.first_name
                        ...
                        : "Trader";
                    return (
                      <tr key={entry.user_id} className={`transition-colors ${isMe ? "bg-(--primary-default)/5" : "hover:bg-white/2"}`}>
                        <td className="px-5 py-3 font-bold text-(--primary-default)">#{i + 1}</td>
                        <td className="px-5 py-3 font-medium">{displayName}</td>
                        <td className="px-5 py-3 text-emerald-400 font-semibold">+{entry.profit_pct.toFixed(2)}%</td>
                        <td className="px-5 py-3 text-(--text-white-50)">{entry.points.toLocaleString()}</td>
                      </tr>
                    );
                  })}
                </tbody>

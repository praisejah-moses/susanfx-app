  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      if (mode === "login") {
        ...
        // Create a user_details record for the newly signed up user
        if (data?.user) {
          try {
            ...
            if (detailsError) {
              console.error("Failed to create user_details:", detailsError);
            }
          } catch (insertErr) {
            console.error("Error inserting user_details:", insertErr);
          }
            // Also update the accounts row (if present) with the user's name
            try {
              ...
              if (acctErr) {
                console.error("Failed to update accounts with name:", acctErr);
              }
            } catch (acctUpdateErr) {
              console.error("Error updating accounts:", acctUpdateErr);
            }
        }
        if (error) {
          setError(error.message);
      ...
      {signupConfirmationEmail && (
        <div className="w-full flex flex-col items-center gap-4 sm:gap-6 animate-fadeInUp">
          ...
          <p className="text-sm text-center text-[var(--text-white-50)]">
            Click the link in the email to confirm your account. You'll be
      ...
      {!signupConfirmationEmail && (
        <>
          ...
          >
            ...
            {mode === "signup" && (
              <div className="w-full flex flex-col gap-2">
                <p className="text-sm font-medium text-[var(--global-text)]">
                  Referral Code{" "}
                  <span className="text-[var(--text-white-50)] text-xs">
                    (Optional)
                  </span>
                </p>
                <div className="relative w-full">
                  <input
                    ...
                    value={refererCode}
                    onChange={(e) =>
                      setRefererCode(e.target.value.toUpperCase())
                    }
                    className="w-full border border-[var(--strokes-default)] rounded-lg px-3 py-[10px] outline-none focus:border-[var(--primary-default)] bg-[var(--background-default)] text-[var(--global-text)] placeholder:text-[var(--text-white-50)] text-sm hover:bg-[var(--background-secondary)] transition-smooth-fast"
                  />
                </div>
          ...
          <div className="text-xs sm:text-sm pb-4 sm:pb-6 px-2 text-[var(--global-text)] text-center">
            ...
            <a
              href="https://home.susanfxtradellc.live/terms"
              className="text-[var(--primary-default)] underline"
            >
              Terms &amp; Conditions
            </a>
          </div>
